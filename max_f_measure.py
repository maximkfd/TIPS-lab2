...
for k in range(0, 10000):
    k1 = k / 10000
    argument.append(k1)
    measure = calculate_f_measure(k1)
    f_array.append(measure)
    # print("r =", k1, "f_measure =", measure)
    if f_measure_max < measure:
        f_measure_max = measure
        f_measure_max_r = k1

f_ndarray = numpy.array(f_array)
plt.plot(argument, f_ndarray, "--r")  # машинная точность для сравнения
plt.xlabel("$r$")
plt.ylabel("$f\;-\;мера$")
plt.show()
print("f_measure_max =", f_measure_max, "при r =", f_measure_max_r)
